...
## The one idea: the schema *is* the type
...
Instead of writing a TypeScript `interface` *and* a separate runtime check that
...
import { characterSchema, type Character, type ActiveChoice } from "@/types/character";

// ❌ never reach past the barrel into an internal file
import { characterSchema } from "@/types/character/character";
```

This is what lets the files stay small *and* keeps imports short: split a domain
...
> **Don't hand-write an interface for anything that comes from the API.** Write a
> Zod schema and infer the type. (CLAUDE.md enforces this.)
